...
# Note: Proxy Client testing with proper endpoints
echo ""
echo -e "${BLUE}[8/10] Verificando Proxy Client...${NC}"

# Test proxy-client health endpoint
if curl -s "http://localhost:8900/actuator/health" > /dev/null 2>&1; then
    echo -e "✅ ${GREEN}Proxy Client Health - OK${NC}"
    
    # Test proxy-client public endpoints  
    echo "   → Testing public endpoints..."
    if curl -s "http://localhost:8900/api/products" > /dev/null 2>&1; then
        echo -e "✅ ${GREEN}Proxy Client Products API - OK${NC}"
    else
        echo -e "⚠️  ${YELLOW}Proxy Client Products API - ERROR${NC}"
    fi
    
    if curl -s "http://localhost:8900/api/categories" > /dev/null 2>&1; then
        echo -e "✅ ${GREEN}Proxy Client Categories API - OK${NC}"
    else
        echo -e "⚠️  ${YELLOW}Proxy Client Categories API - ERROR${NC}"
    fi
    
    # Test authentication endpoint
    echo "   → Testing authentication endpoint..."
    local auth_test=$(curl -s -X POST -H "Content-Type: application/json" \
        -d '{"username":"admin","password":"admin"}' \
        "http://localhost:8900/api/authenticate" \
        -w "%{http_code}")
    
    if [[ "$auth_test" =~ "200" ]]; then
        echo -e "✅ ${GREEN}Proxy Client Authentication - OK${NC}"
    else
        echo -e "⚠️  ${YELLOW}Proxy Client Authentication - Needs valid credentials${NC}"
    fi
    
    # Test Swagger UI
    echo "   → Testing Swagger UI..."
    if curl -s "http://localhost:8900/swagger-ui/index.html" | grep -q "swagger" -i; then
        echo -e "✅ ${GREEN}Proxy Client Swagger UI - OK${NC}"
    else
        echo -e "⚠️  ${YELLOW}Proxy Client Swagger UI - Not accessible${NC}"
    fi
    
    echo -e "✅ ${GREEN}Proxy Client - COMPREHENSIVE TEST COMPLETED${NC}"
    ((SUCCESS_COUNT++))
else
    echo -e "❌ ${RED}Proxy Client - ERROR (Health check failed)${NC}"
    ((ERROR_COUNT++))
fi
